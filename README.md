...
关键字按行处理，空行会被忽略，所有非空行都是必选条件。也就是说，上面三行等价于 `2026-05-08 AND ￥488 AND 内场`，任意一行没有在页面可见文本中匹配到，都不会触发有票报警。
...
如果页面上存在多个相同关键字选项，例如两个 `￥148`，脚本会优先使用页面上已选中的选项；如果无法识别已选中状态，则默认取第一个匹配项。为避免选错，建议在关键字里继续加票档名，例如 `solo电竞酒店住宿票`。
